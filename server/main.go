...
func smartFallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Smart routing: handling request %s (asset: %v, client: %s)", r.URL.Path, isAsset, clientKey)
	...
	if tunnelID := clientTracker.GetBestTunnel(clientKey); tunnelID != "" {
		...
		if confidence > 0.7 && tryTunnelRouteWithTimeout(w, r, tunnelID, isAsset) {
	...
	// No tunnel worked
	log.Printf("Smart routing failed: %s (tried %d tunnels)", r.URL.Path, len(tunnelIDs))
	http.NotFound(w, r)
}
